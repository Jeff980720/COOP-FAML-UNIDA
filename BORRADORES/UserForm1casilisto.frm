...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} UserForm1 
   ...
   ClientTop       =   468
   ClientWidth     =   6216
   OleObjectBlob   =   "UserForm1casilisto.frx":0000
...
Private Sub CommandButton10_Click()
    ...
    Set celda = ws.Range("A:A").Find(What:=CStr(Me.tbsecaj.Value), _
    LookIn:=xlValues, LookAt:=xlWhole, MatchCase:=False)
    
    If celda Is Nothing Then
        MsgBox "El ID que quieres eliminar no exite", vbCritical
    Else
        If IsNumeric(Me.tbsecaj.Text) Then
        Dim respuesta As VbMsgBoxResult
        ...
        If respuesta = vbYes Then
                ...
                If clave <> "1998" Then
                    ...
                    Exit Sub
                End If
            Call EliminarRegistroCajaPorID(CLng(Me.tbsecaj.Text))
            MsgBox "Registro eliminado (si existía).", vbInformation
            Call LimpiarCamposCaja
            Me.tbsecaj.Text = GenerarSiguienteIDCaja("")
        Else
            MsgBox "Eliminación cancelada.", vbInformation
        End If
        Else
            MsgBox "Por favor ingresa un ID válido.", vbExclamation
        End If
    End If
End Sub

Private Sub CommandButton11_Click()
    Me.tbsecaj.Enabled = True
    Me.tbsecaj.SetFocus ' Mueve el cursor al textbox para que el usuario escriba
End Sub
...
Private Sub tbsecaj_Change()

End Sub

Private Sub tbsecaj_KeyDown(ByVal KeyCode As MSForms.ReturnInteger, ByVal Shift As Integer)
    If KeyCode = vbKeyReturn Then
    If IsNumeric(Me.tbsecap.Text) Then
        Call BuscarRegistroCajaPorID(CLng(Me.tbsecaj.Text))
    Else
...
'    nuevoIDAporte = GenerarSiguienteIDAportes("")   ' Asegúrate de que esta función esté definida correctamente
    nuevoIDAporte = Format(GenerarSiguienteIDAportes(""), "")
...
'    Me.tbsecpag.Text = nuevoIDPago
    nuevoIDCaja = Format(GenerarSiguienteIDCaja(""), "") ' Asegúrate de que esta función esté definida correctamente
...
Private Sub cbsaveap_Click()
    ...
    Else
        ...
        Dim respuesta As VbMsgBoxResult
        respuesta = MsgBox("¿Estás seguro de que deseas guardar este registro?", vbYesNo + vbQuestion, "Confirmar eliminación")

        If respuesta = vbYes Then
        
            Set celda = ws.Range("F:F").Find(What:=CStr(Me.tbsecap.Value), _
            LookIn:=xlValues, LookAt:=xlWhole, MatchCase:=False)
            ...
            If celda Is Nothing Then
                ...
                End With
            Else
                MsgBox "El codigo ya existe, por favor ingrese otro codigo " & Me.tbsecap.Text
            End If
        Else
            MsgBox "Eliminación cancelada.", vbInformation
        End If
...
'    MsgBox "¡Datos guardados correctamente!" & Me.tbsecap.Text, vbInformation
...
Private Sub cbsavecj_Click()
    ...
    Else
        ...
        End With
        Else
            MsgBox "El codigo ya existe, por favor ingrese otro codigo " & Me.tbsecaj.Text
        End If
        Else
         MsgBox "Eliminación cancelada.", vbInformation
        End If
...
'        Set celda = ws.Range("A:A").Find(What:=valorBuscado, After:=ws.Range("A3"))
    ...
    If celda Is Nothing Then
        ...
        MsgBox "Registro guardado satisfactoriamente " & valorBuscado
        End With
    Else
        MsgBox "El codigo ya existe, por favor ingrese otro codigo " & valorBuscado
    End If
        Call LimpiarCamposFormularioPagos
End If
    Sheets("PAGOS").Select
...
'        If IsNumeric(fila.Range(1, 1).Value) Then
            If fila.Range(1, 1).Value = idBuscado Then
'            If CLng(fila.Range(1, 1).Value) = idBuscado Then
                Me.tbcodsocio = fila.Range(1, 2).Value
'                MsgBox "dato: " & tbcodsocio.Value
                Me.cmbnomsocpg = fila.Range(1, 3).Value
                MsgBox "dato: " & cmbnomsocpg.Value
...
End Sub
Sub BuscarRegistroCajaPorID(idBuscado As Long)
    Dim ws As Worksheet
    Dim tbl As ListObject
    Dim fila As ListRow
    ...
    For Each fila In tbl.ListRows
        If IsNumeric(fila.Range(1, 1).Value) Then
              If CLng(fila.Range(1, 1).Value) = idBuscado Then
                Me.tbfechcaj.Text = fila.Range(1, 2).Value
                Me.tbmontcj.Text = Replace(fila.Range(1, 3).Value, ",", ".")
                Me.tbcmtcj.MultiLine = True
                Me.tbcmtcj.Text = fila.Range(1, 4).Value
                Exit Sub
            End If
        End If
    Next fila
...
Sub EliminarRegistroAportePorID(idBuscado As Long)
    ...
    ws.Protect Password:="1998"
End Sub
Sub EliminarRegistroCajaPorID(idBuscado As Long)
    Dim ws As Worksheet
    Dim tbl As ListObject
    Dim fila As ListRow
...
'            Call EliminarRegistroPagosPorID(valorBuscado)
            ...
            MsgBox "Registro eliminado (si existía)." & valorBuscado, vbInformation
            Call LimpiarCamposFormularioPagos
'            Me.tbsecpag.Text = GenerarSiguienteIDPrest("")
            Else
'            Call EliminarRegistroPagosPorID(Me.tbsecpag.Value)
            Call EliminarRegistroPagosPorID(valorBuscado)
            MsgBox "Registro eliminado (si existía).", vbInformation
            Call LimpiarCamposFormularioPagos
'            Me.tbsecpag.Text = GenerarSiguienteIDPrest("")
